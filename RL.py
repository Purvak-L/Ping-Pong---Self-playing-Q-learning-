...
#hyper params
Number_of_actions = 3 #up,down, stay
...
def convArch():
    ...
    W_fc5 = tf.Variable(tf.zeros([784, Number_of_actions]))
    b_fc5 = tf.Variable(tf.zeros([Number_of_actions]))
...
def QNetwork(inp, out, sess):
	argmax = tf.placeholder("float", [None, Number_of_actions]) 
    ...
    while(1):
        ...
        #argmax function
        argmax_t = np.zeros([Number_of_actions])

        #
        if(random.random() <= epsilon):
            maxIndex = random.randrange(Number_of_actions)
        else:
            maxIndex = np.argmax(out_t)
        argmax_t[maxIndex] = 1
        
        if epsilon > rate_final:
            epsilon -= (rate_intial - rate_final) / EXPLORE
